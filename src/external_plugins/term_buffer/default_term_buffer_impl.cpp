...
void DefaultTermBuffer::DeleteLines(uint32_t begin, uint32_t count, const TermCellPtr & cell_template) {
    std::lock_guard<std::recursive_mutex> guard(m_UpdateLock);
    m_Buffers[m_CurBuffer].DeleteLines(begin, count, cell_template);
}

void DefaultTermBuffer::InsertLines(uint32_t begin, uint32_t count, const TermCellPtr & cell_template) {
    std::lock_guard<std::recursive_mutex> guard(m_UpdateLock);
    m_Buffers[m_CurBuffer].InsertLines(begin, count, cell_template);
}

void DefaultTermBuffer::ScrollBuffer(int32_t scroll_offset, const TermCellPtr & cell_template) {
    std::lock_guard<std::recursive_mutex> guard(m_UpdateLock);
    m_Buffers[m_CurBuffer].ScrollBuffer(scroll_offset, cell_template);
}

bool DefaultTermBuffer::MoveCurRow(uint32_t offset, bool move_down, bool scroll_buffer, const TermCellPtr & cell_template) {
    std::lock_guard<std::recursive_mutex> guard(m_UpdateLock);
...
void DefaultTermBuffer::SetCurCellData(uint32_t ch, bool wide_char, bool insert, const TermCellPtr & cell_template) {
    std::lock_guard<std::recursive_mutex> guard(m_UpdateLock);
