...
void ScintillaEditorDataHandlerImpl::LoadNativeDataHandler() {
    std::string termcap_dir = GetPluginContext()->GetAppConfig()->GetEntry("/term/termcap_dir", "data");
    std::string term_name = GetPluginContext()->GetAppConfig()->GetEntry("/term/term_name", "scintilla-editor");
    ...
    if (load_cap_str(termcap_dir, term_name, term_entry)) {
        ...
        std::cerr << "load scintilla editor caps failed!" << std::endl;
        throw std::invalid_argument("load scintilla editor caps failed!");
    }
...
void ScintillaEditorDataHandlerImpl::ProcessSingleCharNative(const char * ch) {
    if (!ch) {
        if (!m_State)
            return;
...
void ScintillaEditorDataHandlerImpl::HandleCap(bool check_unknown, char c) {
    CapNameMapValue cap_tuple;
    (void)c;
