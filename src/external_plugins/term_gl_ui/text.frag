...
void main()
{
    ...
    if( pixel.z == 1.0)
    {
        float a = texture(tex, vtex_coord).r;
        output_color = vcolor * pow( a, 1.0/vgamma );
        return;
    }

    // LCD On
    vec4 current = texture(tex, vtex_coord);
    vec4 previous= texture(tex, vtex_coord+vec2(-1.,0.)*pixel.xy);
    vec4 next    = texture(tex, vtex_coord+vec2(+1.,0.)*pixel.xy);
   ...
   output_color = vec4( color.rgb, vcolor.a*color.a);
}
